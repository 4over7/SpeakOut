...
各家默认模型跑完后，对「是否该换默认」做了针对性对比。

**智谱** —— `glm-4-flash` 虽只有 6/8，但换代价太大，**保持不动**：

| 模型 | 通过 | TTFT 中位 |
|---|---|---|
| glm-4-flash（现默认） | 6/8 | **614ms** |
| glm-4.5-air | 8/8 | 2448ms（且把「扣S」擅自改成「KPI」，过度改写） |
| glm-5.2 / glm-5-turbo / glm-4.6 | 8/8 | 3271 / 4668 / 7338ms |

**豆包** —— 保持 `doubao-seed-2-0-mini-260215`：

| 模型 | 通过 | 总时中位 |
|---|---|---|
| doubao-seed-2-0-mini-260215（现默认） | 8/8 | **1215ms** |
| doubao-seed-2-1-turbo-260628 | — | 开推理 **10213ms** / 关掉 1429ms |
| doubao-seed-2-0-mini-260428 | 8/8 | 1546ms |

`2-1-turbo` 是**推理模型**，流式输出走 `reasoning_content` 而非 `content`，
不显式关 thinking 就要 10 秒；即便关掉（1429ms）也不比现役快。

**Kimi** —— 默认从 `kimi-k2.5` 改为 **`kimi-k2.6`**：

| 模型 | 通过 | 总时中位 |
|---|---|---|
| kimi-k2.5（原默认） | 8/8 | 13032ms |
| **kimi-k2.6**（现默认） | 8/8 | 6791ms（峰值 23.5s） |
| kimi-k3 | — | 同样只接受 temperature=1 |
| moonshot-v1-8k | 7/8 | **881ms** |

k2.6 比原默认快一倍且质量满分。`moonshot-v1-8k` 快 15 倍但属 v1 老系列，
且唯一失败项（把 `vector database` 译成中文）是各家通病，非它独有。
**Kimi 整体仍不适合做兜底推荐** —— 峰值 23 秒撑不起「松开即出字」。
