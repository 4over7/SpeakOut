...
**本体系之外还有一份 [`README.md`](./README.md)**：面向**用户和外部访客**（功能介绍、安装、截图、模型清单、中英双语），不是给 agent 看的。
改动会影响用户可见行为时（模式、功能、权限、支持的 provider），**README 也要同步** —— 它曾整整落后两个版本，还在宣传 v1.9.0 已删除的「智能模式」。
